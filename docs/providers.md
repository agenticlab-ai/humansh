...
Interactive setup always asks which provider to use, with the saved choice shown only as the default. It probes only the selected CLI provider. Setup never starts a login flow. If the live check fails, setup shows the bounded provider error and lets the user fix and retry that same provider without restarting installation. Answering no returns to the provider list; cancelling or exhausting a non-interactive setup exits nonzero before saving credentials, configuration, or shell files, allowing an invoking installer to roll back its binary replacement.
